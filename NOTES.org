...
*** Logic change    
    ...
    + We shall do this by having an intermediate buffer
...
**** New approach to doing the Slit
     thanks to the $backpermute$ function, we can do the following:
     + We compute a slit image using $backpermute$
     + We concatenate that image with the existing intermediate.
     + We crop the new intermediate image to be the correct size.
