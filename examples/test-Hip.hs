...
main = do
  let down2x2 = Sz (3300 :. 2100)
      ...
      down = Sz (75 :. 240)
      up = Sz (125 :. 400)
      addJP :: Image (A.SRGB A.Linear) Double -> (Image (A.SRGB A.Linear) Float, JP.Image JP.PixelRGB8)
      addJP img = (I.map (fmap toFloat) img, M.toJPImageRGB8 $ A.toImageBaseModel a)
        where
          JP.Image a = I.map (fmap toWord8) img
  defaultMain
    ...
    , env (readImageRGB "images/downloaded/frog-1280x824.jpg") $ \img ->
        bgroup
          "Shrink2x2"
          [ bench "resize" $ nf (resize Bilinear (I.Fill 0) down2x2) img
          , bench "resizeDW" $ nf (resizeDW Bilinear (I.Fill 0) down2x2) img
    ...
    , env (readImageRGB "/home/lehins/tmp/frog-6600x4200.jpg") $ \img ->
        bgroup
          "Shrink4x4"
          [ bench "resize" $
            nf (resize Bilinear (I.Fill 0) down4x4 . resize Bilinear (I.Fill 0) down2x2) img
          , bench "resizeDW" $
            nf (resizeDW Bilinear (I.Fill 0) down4x4 . resizeDW Bilinear (I.Fill 0) down2x2) img
...
benchBilinearResize :: Sz2 -> (Image (SRGB 'Linear) Float, Image JP.PixelRGB8) -> Benchmark
benchBilinearResize sz@(Sz2 m n) ~(i, jp) =
  bgroup
    ("Bilinear " ++ show sz)
    [ bench "HIP" $ nf (resize Bilinear (I.Fill 0) sz) i
    , bench "JuicyPixels-extra" $ nf (scaleBilinear n m) jp
    ]

benchBicubicResize :: Sz2 -> Image (A.SRGB A.Linear) Float -> Benchmark
benchBicubicResize sz img =
  bgroup
    ("Bicubic " ++ show sz)
    [bench "HIP" $ nf (resize (Bicubic 0.5) (I.Fill 0) sz) img]
